...
function NextBestActions() {
  return <Panel title="Next Best Actions" icon="auto_awesome" className="md:col-span-2"><div className="divide-y divide-outline-variant">{actions.map(([id, customer, action, confidence]) => <div key={id} className="flex items-center gap-4 py-4 first:pt-0 last:pb-0"><div className="w-10 h-10 rounded-full bg-primary/10 text-primary flex items-center justify-center"><span className="material-symbols-outlined">person</span></div><div className="flex-1"><Link to={`/customer-720?customerId=${id}`} className="font-semibold text-primary hover:underline">{customer}</Link><p className="text-sm text-on-surface-variant">{action}</p></div><span className="text-xs font-bold text-primary bg-primary/10 px-2 py-1 rounded-full">{confidence} confidence</span><button className="text-primary font-semibold text-sm">Start</button></div>)}</div></Panel>;
}
...
function Gamification({ icon, label, value, note }) { return <div className="rounded-xl p-5 bg-gamification-bg border border-gamification-border/30"><span className="material-symbols-outlined text-gamification-accent">{icon}</span><p className="mt-3 text-xs font-bold uppercase tracking-wider text-gamification-text">{label}</p><p className="mt-1 text-2xl font-bold text-gamification-text-dark">{value}</p><p className="mt-1 text-xs text-gamification-text">{note}</p></div>; }

function TeamLeaderboard() { const rows = advisors.slice().sort((a, b) => b.monthlySales - a.monthlySales).slice(0, 10).map((a) => [a.name, `$${(a.monthlySales / 1000).toFixed(0)}k`, `$${(a.premiumCollected / 1000).toFixed(1)}k`, Math.round(a.monthlySales / 2200)]); return <Panel title="Team Leaderboard" icon="leaderboard" className="md:col-span-2"><Table headers={['Advisor', 'Sales', 'Premium', 'Score']} rows={rows} /></Panel>; }
...
function AgencyPerformanceRanking() { const rows = agencySummary().map((a) => [a.name, `$${(a.revenue / 1000).toFixed(0)}k`, `${a.target}%`]); return <Panel title="Agency Performance Ranking" icon="business" className="md:col-span-2"><Table headers={['Agency', 'Revenue', 'Target']} rows={rows} /></Panel>; }
function TopVsLowestAgency() { return <Panel title="Top vs. Lowest Agency" icon="compare_arrows"><div className="grid gap-3"><Compare label="Top performer" agency="Downtown Toronto" value="104%" color="text-primary bg-primary/10" /><Compare label="Needs attention" agency="Markham" value="88%" color="text-gamification-text-dark bg-gamification-bg" /></div></Panel>; }
...
function Panel({ title, icon, children, className = '' }) { return <section className={`bg-white rounded-xl border border-outline-variant shadow-sm p-6 ${className}`}><h2 className="flex gap-2 items-center text-lg font-bold mb-5"><span className="material-symbols-outlined text-primary">{icon}</span>{title}</h2>{children}</section>; }
...
export default function RoleDashboard() {
  ...
  return <div className="p-4 md:p-8 max-w-[1600px] mx-auto"><header className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-8"><div><p className="text-sm font-bold text-primary uppercase tracking-wider">{role}</p><h1 className="text-3xl font-bold text-on-surface mt-1">{config.title}</h1><p className="text-on-surface-variant mt-2">A focused view of the work that matters to you today.</p></div><button className="self-start sm:self-auto px-4 py-2 rounded-lg bg-primary text-white font-semibold">View report</button></header><section className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-8 gap-4 mb-8">{derivedKpis(role).map(([label, value, detail]) => <div key={label} className="bg-white rounded-xl border border-outline-variant p-4 shadow-sm"><p className="text-xs text-on-surface-variant font-semibold">{label}</p><p className="text-2xl font-bold text-on-surface mt-3">{value}</p><p className="text-xs text-primary mt-2">{detail}</p></div>)}</section><section className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">{config.widgets.map((name) => { const Widget = widgets[name]; return <Widget key={name} />; })}</section></div>;
}
