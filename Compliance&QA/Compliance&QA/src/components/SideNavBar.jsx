...
const navItems = [
  ...
  { icon: 'map', label: 'Advisor Journey', to: '/advisor-journey' },
  { icon: 'person_add', label: 'Leads', to: '/leads' },
  { icon: 'group', label: 'Customer Base', to: '/customer-base' },
]
    ...
    <aside className="hidden lg:flex flex-col h-full py-6 fixed left-0 top-0 h-full w-[280px] bg-primary dark:bg-primary-fixed-dim text-on-primary shadow-lg z-50">
      ...
      <nav className="flex-1 space-y-1">
        ...
        <NavLink
          ...
          className={({ isActive }) =>
            isActive
              ? 'flex items-center gap-3 bg-primary-container text-on-primary-container rounded-lg px-4 py-3 mx-2 mt-10'
              : 'flex items-center gap-3 text-on-primary/80 hover:text-on-primary hover:bg-on-primary/10 transition-colors px-4 py-3 mx-2 rounded-lg mt-10'
          }
        >
